    ...
    */
    public static void main(String[] args) {
        ...
        for(int i = 0; i < nUndergrad; i++) {
            users[i] = new User(i, UserType.Undergrad, tutor);
            threads[i] = new Thread(users[i]);
            threads[i].start();
        }
        for(int i = 0; i < nGrad; i++) {
            users[nUndergrad + i] = new User(nUndergrad + i, UserType.Grad, tutor);
            threads[nUndergrad + i] = new Thread(users[nUndergrad + i]);
            threads[nUndergrad + i].start();
        }
        for(int i = 0; i < nProf; i++) {
            users[nUndergrad + nGrad + i] = new User(nUndergrad + i, UserType.Professor, tutor);
